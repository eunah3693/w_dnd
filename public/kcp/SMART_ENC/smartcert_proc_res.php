<?
    ...
    // 결과 처리
        if( $res_cd == "0000" )
        {
            ...
            session_start();
            $_SESSION['phone_no'] = $ct_cert->mf_get_key_value("phone_no");
            $_SESSION['user_name'] = $ct_cert->mf_get_key_value("user_name");
            $_SESSION['birth_day'] = $ct_cert->mf_get_key_value("birth_day");
            $_SESSION['sex_code'] = $ct_cert->mf_get_key_value("sex_code");
            ...
            $ret['code'] = '00';
            $ret['phone_no'] = $ct_cert->mf_get_key_value("phone_no");
            $ret['user_name'] = $ct_cert->mf_get_key_value("user_name");
            $ret['birth_day'] = $ct_cert->mf_get_key_value("birth_day");
            $ret['sex_code'] = $ct_cert->mf_get_key_value("sex_code");
            $ret['msg'] = '인증완료되었습니다.';
    ...
    <head>
        ...
        <script type="text/javascript">
            window.onload=function()
            {
                try
                {
                    if( ( navigator.userAgent.indexOf("Android") > - 1 || navigator.userAgent.indexOf("iPhone") > - 1 || navigator.userAgent.indexOf("iOS") > - 1 ) ) // 스마트폰인 경우
                    {
                        parent.auth_data( <?= json_encode($ret) ?> ); // 부모창으로 값 전달
                    }
                    else // 스마트폰 아닐때
                    {
                        console.log(<?= json_encode($ret) ?>);
                        opener.auth_data( <?=  json_encode($ret) ?> ); // 부모창으로 값 전달
                        self.close();
                    }
                }
                catch(e)
