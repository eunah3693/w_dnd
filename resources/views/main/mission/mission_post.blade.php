...
@section('content')
<form action="/api/post/update" method="POST" id="mission_form">
    <section>
        <div class="mission-description">
            <!-- <h2>미션 안내</h2> -->

            <h3 class="md-h2">{!! preg_replace("/\]/", "]</span>", preg_replace("/\[/", "<span>[", $data->missionPool->title)) !!}</h3>
            <p>
                {{ $data->missionPool->sub_title }}
            </p>
        ...
        <div class="pet-filter clearboth">
            @foreach ($pet as $item)
                <label class="box-radio-input">
                    <input type="checkbox" name="pet_idx[]" @if (!$data->pet_idx){{ $loop->index == 0 ? 'checked':'' }}@endif value="{{ $item->idx }}">
                    <div>
                        @if ($item->file_idx)
        ...
        <div class="cont-box1">
            <h2>글 작성</h2>
            <div class="txt-box">
                <textarea id="textarea" name="content" class="textarea" rows="12" placeholder="글을 작성 해 주세요"></textarea>
                <span class="tag-box">
                    @foreach ( explode(', ',$data->missionPool->tag) as $item)
                    #{{ $item }}
                    @endforeach
                </span>
            </div>
        ...
        <a class="save-btn" id="btn_popup_open" onclick="missionSubmit({{$post->idx}})">미션 최종 제출!</a>
