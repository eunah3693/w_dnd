...
@section('content')
                ...
                <div class="content" style="max-width:1400px">
                    ...
                    <div class="block block-rounded">
                        <div class="block-header block-header-default">
                            ...
                            <div class="block-options" onclick="location.href='/admin/member/level_modify'">
                        ...
                        <div class="block-content">
                            {{-- 검색창 --}}
                            <form action="/admin/member/level" method="get">
                                ...
                                <table class="table table-borderless table-striped table-vcenter">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="d-none d-sm-table-cell text-center" style="vertical-align:middle"><span>경험치</span></th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="d-none d-sm-table-cell text-center font-size-sm" >{{ $t->exp }}</td>
                                        <td class="d-none d-sm-table-cell text-left font-size-sm" style="padding-left:20px" onclick="location.href='/admin/member/level_detail?exp_idx={{ $t->idx }}'">{{ $t->memo }}</td>
                                        <td class="d-none d-sm-table-cell text-center font-size-sm" >{{ $t->created_at }}</td>
                                        <td class="d-none d-sm-table-cell text-center font-size-sm cursor" onclick="location.href='/admin/member/level_modify?exp_idx={{ $t->idx }}'"  ><i class="si si-pencil fa-fw"></i></td>
                            ...
                            {{ $exp->appends(request()->input())->links() }}
                            </div>
                            {{-- END Pagination --}}
                        </div>
                    </div>
        ...
        <script>
            ...
            {
                if(confirm('경험치를 정말 삭제 하시겠습니까?'))
                {
                    var data = {
                        exp_idx : idx
                    }
                    $.post('/api/admin/exp/delete', data, function(res){
                        if(res.status == '200')
                        {
                            alert(res.msg);
                            location.reload();
