        ...
        <div class="content" style="max-width:1400px">
            ...
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    <h3 class="block-title">전체 배너리스트</h3>
                        <div class="block-options"  onclick="location.href='/admin/manage/popup_modify'">
                        <button type="submit" class="btn btn-dark" id="btn_add" data-btn="popup">추 가</button>
                         </div>
                </div>
                <div class="block-content">

                    {{-- 검색창--}}
                    <form action="/admin/manage/popup" method="get">
                    ...
                    <div class="table-responsive">
                        <table class="table table-borderless table-striped table-vcenter" id="table_list" data-table="popup">
                            <thead>
                                <tr>
                                    <th class="vertical_center text-center" style="vertical-align:middle"><span>순번</span></th>
                                    <th class="d-none d-sm-table-cell text-center" style="vertical-align:middle"><span>위치</span></th>
                                    <th class="text-center" style="vertical-align:middle"><span>제목</span></th>
                                    <th class="d-none d-xl-table-cell text-center" style="vertical-align:middle"><span>순서</span></th>
                                    <th class="d-none d-sm-table-cell text-center" style="vertical-align:middle"><span>노출날짜</span></th>
                            ...
                            <tbody>
                                ...
                                <tr >
                                    ...
                                    <td class="d-sm-table-cell text-center font-size-sm">{{ $val->page }}</td>
                                    <td class="font-w600 cursor" onclick="location.href='/admin/manage/popup_detail?idx={{ $val->idx }}'">{{ $val->title }}</td>
                                    <td class="text-center">{{ $val->order }}</td>
                                    <td class="d-xl-table-cell text-center font-size-sm">{{ $val->startdate }} ~ {{ $val->enddate }}</td>
                                    <td class="d-xl-table-cell text-center font-size-sm">
                                        @if ($val->is_public == 1)
                                        <i onclick="updatePublic({{$val->idx}}, 0)" class="fa fa-fw fa-eye"></i>
                                        @else
                                        <i onclick="updatePublic({{$val->idx}}, 1)" class="far fa-fw fa-eye-slash"></i>
                                        @endif
                                    </td>
                                    <td class="d-xl-table-cell text-center font-size-sm  cursor" onclick="location.href='/admin/manage/popup_modify?idx={{ $val->idx }}'"><i class="si si-pencil fa-fw"></i></td>
        ...
        function updatePublic(idx, type)
            {
                ...
                $.post('/api/admin/popup/visible', data, function(res){
                    if(res.status == 200)
                    {
                        alert(res.msg);
                        location.reload();
            ...
            function deleteData(idx)
            {
                if(confirm('삭제하시겠습니까?'))
                {
                    ...
                    $.post('/api/admin/manage/popup/delete', data, function(res){
                        if(res.status == 200)
                        {
                            alert(res.msg);
                            location.reload();
